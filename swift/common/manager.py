...
class Manager(object):
    ...
    def start(self, **kwargs):
        ...
        # kwargs是 swift-init:main() 中 options 传过来的参数，eg:
        # {'run_dir': '/var/run/swift', 'daemon': True, 'verbose': False,
        #  'kill_wait': 15, 'number': 0, 'strict': None,'graceful': False,
        #  'once': False, 'kill_after_timeout': None, 'wait': True}
    ...
    # cmd = start or stop
    # {'run_dir': '/var/run/swift', 'daemon': True,
    # 'verbose': False, 'kill_wait': 15, 'number': 0,
    # 'strict': None, 'graceful': False, 'once': False,
    # 'kill_after_timeout': None, 'wait': True}
    ...
    def wait(self, **kwargs):
        ...
        for proc in self.procs:
            ...
            if kwargs.get('once', False):
                ...
                # 如果 once 为 True，父进程等待子进程结束后再往下运行
                proc.wait()
            if output:
                print(output)
                start = time.time()
                ...
                while time.time() - start < WARNING_WAIT:
                    time.sleep(0.1)
                    # poll()用于检查子进程是否已经结束,设置并返回 returncode 属性
