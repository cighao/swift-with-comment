...
def eventlet_monkey_patch():
    ...
    # eventlet.patcher.monkey_patch() 是在运行时修改已有的代码，动态替换已有的标准库
    # 如果调用该方法时没有指定参数，会为所有默认参数中提到的库打补丁
    # 关键字参数指示哪些模块需要被打补丁，如果 all 是真，那么所有的模块会被打补丁而无视其他参数；
    # 否则才由具体模块对应的参数控制对指定模块的补丁。多数参数为与自己同名的模块打补丁
...
class Timestamp(object):
    """
    Internal Representation of Swift Time. (swift的内部时间)
    ...
    def __init__(self, timestamp, offset=0, delta=0):
        ...
        if isinstance(timestamp, six.string_types):
            # partition()如果字符串包含指定的分隔符，则返回一个3元的元组，
            # 第一个为分隔符左边的子串，第二个为分隔符本身，第三个为分隔符右边的子串。
            # seq[start:end:step], [::2],从头取到尾，间隔为2
...
class ContextPool(GreenPool):
    ...
    # 紧跟 with 后面的语句被求值后，调用 __enter__()，返回值赋值给as后面的变量
    def __enter__(self):
        return self

    # 当 with后面的代码块全部被执行完之后，调用对象的__exit__()方法
...
class GreenAsyncPile(object):
    ...
    def _wait(self, timeout, first_n=None):
        ...
        try:
            with GreenAsyncPileWaitallTimeout(timeout):
                while True:
                    # 从 _responses 中取出元素放到 results 中
                    results.append(next(self))
                    # first_n > , 只取 _responses 中的前 n 个
                    # first = None, 取 _responses 中的所有个
    ...
    def next(self):
        try:
            # get_nowait() 采用非阻塞方式从队列中取元素，一旦队列为空，则会引发Empty异常
            rv = self._responses.get_nowait()
        except eventlet.queue.Empty:
            if self._inflight == 0:
                raise StopIteration()
            # 采用阻塞方式从队列中取元素,如果队列为空，则线程会停下来等待队列中有元素可以取出
