...
class ObjectController(BaseStorageServer):
    ...
    def __init__(self, conf, logger=None):
        ...
        self.slow = int(conf.get('slow', 0)) # If > 0, Minimum time in seconds for a PUT or DELETE request to complete
    ...
    def __call__(self, env, start_response):
        ...
        start_time = time.time()
        # env structure:
        # {'SCRIPT_NAME': '', 'REQUEST_METHOD': 'REPLICATE', 'PATH_INFO': '/sdk/73210',
        #  'SERVER_PROTOCOL': 'HTTP/1.0', 'REMOTE_ADDR': '10.0.0.9', 'CONTENT_LENGTH': '0',
        #  'HTTP_USER_AGENT': 'object-replicator 2928237', 'eventlet.posthooks': [],
        #  'RAW_PATH_INFO': '/sdk/73210', 'REMOTE_PORT': '40794',
        #  'eventlet.input': <eventlet.wsgi.Input object at 0x7f61182f2210>,
        #  'wsgi.url_scheme': 'http', 'SERVER_PORT': '6200', 'HTTP_X_BACKEND_STORAGE_POLICY_INDEX': '0',
        #  'wsgi.input': <eventlet.wsgi.Input object at 0x7f61182f2210>, 'HTTP_HOST': '10.0.0.7:6200',
        #  'wsgi.multithread': True, 'wsgi.version': (1, 0), 'SERVER_NAME': '10.0.0.7',
        #  'GATEWAY_INTERFACE': 'CGI/1.1', 'wsgi.run_once': False,
        #  'wsgi.errors': <swift.common.utils.LoggerFileObject object at 0x7f611837a750>,
        #  'wsgi.multiprocess': False, 'CONTENT_TYPE': None, 'HTTP_ACCEPT_ENCODING': 'identity'}
        req = Request(env)
        ...
        if not check_utf8(req.path_info): #不支持UTF8
            ...
            try:
                ...
                if req.method not in self.allowed_methods: #判断方法是否是共有的方法
                    res = HTTPMethodNotAllowed()
                else:
                    res = getattr(self, req.method)(req)  #执行方法,例如 PUT,get 等
        ...
        if req.method in ('PUT', 'DELETE'):
            ...
            if slow > 0: # slow: minimum time in seconds for a PUT or DELETE request to complete
                sleep(slow)
