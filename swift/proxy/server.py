...
class Application(object):
    ...
    # env 包含了CGI环境变量的环境字典，由服务器对来自客户端的每一次请求而生成。
    # start_response 是由服务器提供的一个回调函数，用以返回HTTP状态码和HTTP响应头
    ...
    def __call__(self, env, start_response):
        ...
        try:
            if self.memcache is None:  #如果缓存为空，缓存env
                # self.memcache = env.get('swift.cache',None)
                self.memcache = cache_from_env(env, True)
            # update 'x-storage-token' of req.header
    ...
    def set_node_timing(self, node, timing):
        if not self.sorts_by_timing:
            return
        now = time()
        timing = round(timing, 3)  # sort timings to the millisecond, round(timing, 3)四舍五入保留三位小数
...
# app_factory方法 实例化功能类(Application)
