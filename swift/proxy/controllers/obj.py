...
class ObjectControllerRouter(object):
    ...
    # classmethod 修饰符对应的函数不需要实例化可以直接调用，
    # eg. ObjectControllerRouter.register()
    # 该函数不需要 self 参数，但第一个参数需要是表示自身类的 cls 参数，
    # 可以cls来调用类的属性，类的方法，实例化对象等。
    # 同时 register() 在这里充当了装饰器的角色
    @classmethod
    ...
    # 可以用 ObjectControllerRouter['key']的形式调用__getitem__()
...
class BaseObjectController(Controller):
    ...
    def _get_put_connections(self, req, nodes, partition, outgoing_headers,
        ...
        # 该对象是一个可以向其中填充工作的迭代器，便于以后从其中读取结果
        ...
        # 从给定的 list中选择出满足if条件的元素组成新的 list
        # 从 pile 中选择不为空的 putter 组成 putters
    ...
    def PUT(self, req):
        ...
        # container_info is a dict, structure :
        # {'status': 204, 'sync_key': None, 'write_acl': None, 'versions': None, 'storage_policy': 1,
        #  'partition': 94513, 'bytes': 1048576, 'meta': {}, 'sharding_state': 'unsharded',
        #  'cors': {'allow_origin': None, 'expose_headers': None, 'max_age': None}, 'nodes': [{...},{...},{...}],
        #  'sysmeta': {}, 'object_count': 1, 'read_acl': None}
        container_info = self.container_info(
            self.account_name, self.container_name, req)
        policy_index = req.headers.get('X-Backend-Storage-Policy-Index',
                                       container_info['storage_policy'])
        ...
        # container_partition: container 的 partition 编号
        # container_nodes: container 所在的 node 的列表
        # container_path: none
        container_partition, container_nodes, container_path = \
            self._get_update_target(req, container_info)

        # partition：is an integer,the partition number
        # nodes: is list of several node
        # node struct: {'index': 0,'replication_port': 6200, 'weight': 100.0, 'zone': 1,
        #               'ip': '10.0.0.7', 'region': 1, 'id': 0, 'replication_ip': '10.0.0.7',
        #               'meta': '', 'device': 'sdk', 'port': 6200}
        partition, nodes = obj_ring.get_nodes(
            self.account_name, self.container_name, self.object_name)
        ...
        def reader():
            try:
                # client_chunk_size can be configured, 65536 by default
                return req.environ['wsgi.input'].read(
                    self.app.client_chunk_size)
            except (ValueError, IOError) as e:
                raise ChunkReadError(str(e))
        # iter 创建了一个迭代器对象，
        # 每次调用这个迭代器对象的__next__()方法时，都会调用 reader
        # 不停的调用 chunk = next(data_source) 可以每次读取 client_chunk_size 字节的数据到chunk中
...
# 副本策略对应的object controller类
# DEFAULT_POLICY_TYPE = REPL_POLICY = 'replication'
# 使用 ObjectControllerRouter.register() 函数作为装饰器
...
def chunk_transformer(policy):
    ...
    buf = collections.deque()  # deque()定义了双端队列，可以从头/尾两端添加或删除元素
    total_buf_len = 0

    # 第一次调用时，在这里暂停
    # 下次调用 .send(msg) 时，chunk=msg，然后往后运行
    chunk = yield
    while chunk:
        buf.append(chunk)   #头部插入
        ...
        if total_buf_len >= segment_size:
            ...
            #  ...]
            # obj_data 是一个列表，列表的第一个元素是 A1,A2,.. 拼接而成的，
            #                          第二个元素是 B1,B2,...拼接而成的，...
            # obj_data 中的每一个元素都对应存到一个 node 上
            obj_data = [''.join(frags)
                        for frags in zip(*frags_by_byte_order)]
            # 迭代到这里时，返回 obj_data
            # 下次调用 .send(msg) 时，chunk = msg,然后继续往下运行
    ...
    # 对最后不足一个 segment_size 大小的数据进行编码
    last_bytes = ''.join(buf)
    if last_bytes:
        last_frags = policy.pyeclib_driver.encode(last_bytes)
        yield last_frags
    else:
        # 如果数据大小刚好是 segment_size 的整数倍，则返回 ['','','','',..] 表示结束
        # policy.ec_n_unique_fragments = data fragments + parity fragments
...
class ECObjectController(BaseObjectController):
    ...
    def _transfer_data(self, req, policy, data_source, putters, nodes,
        ...
        # 第一次调用 chunk_transformer()并不会执行改函数而是生成一个迭代器
        ...
        def send_chunk(chunk):
            ...
            # object server.
            if etag_hasher:
                etag_hasher.update(chunk)  # 对原始数据进行hash
            # 调用 chunk_transform.send() 对数据进行编码
            # backend_chunks 中的每一个元素都对应存到一个 node 上
            ...
            for putter in list(putters):
                ...
                if not putter.failed:
                    ...
                    if frag_index not in updated_frag_indexes:
                        frag_hashers[frag_index].update(backend_chunk) # hash编码后的数据
                        updated_frag_indexes.add(frag_index)
                    putter.send_chunk(backend_chunk)  # put 数据到 putter 的队列中
                else:
                    putter.close()
                    putters.remove(putter)
            self._check_min_conn(
        ...
        try:
            # ContexPool 继承了 GreenPool，实现了方法 __enter__() 和 __exit__()
            # 紧跟 with 后面的语句被执行后，调用 __enter__() 方法，返回值赋值给 as 后面的变量
            # 当 with 后面的代码块全部被执行完之后，将调用 __exit__() 方法
            with ContextPool(len(putters)) as pool:
                ...
                # 调用 _send_file 从 putter 的队列中读取数据发送出去
                for putter in putters:
                    putter.spawn_sender_greenthread(
                        pool, self.app.put_queue_depth, self.app.node_timeout,
                        self.app.exception_occurred)
                while True:
                    with ChunkReadTimeout(self.app.client_timeout):
                        try:
                            # 每次读取 client_chunk_size 字节的数据到chunk中
                            chunk = next(data_source)
                        except StopIteration:
                            break
                    bytes_transferred += len(chunk)
    ...
    def _store_object(self, req, data_source, nodes, partition,
        ...
        if req.content_length:
            ...
            # req.content_length: object size in Byte
            # policy.ec_segment_size: configured in swfit.conf, 1048576 by default
            # a segment is a series of consecutive HTTP chunks buffered up
            # ...before performing an EC operation
            num_fragments = req.content_length // policy.ec_segment_size
            # fragment: Data and parity ‘fragments’ are generated
            # ...when erasure coding transformation is applied to a segment.
            ...
            if last_segment_size:
                # get_segment_info(data_len, segment_size):
