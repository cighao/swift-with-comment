...
def _get_info_from_infocache(env, account, container=None):
    ...
    """
    # get the cache string: object/account_name/container_name/object_name  or
    #                       container/account_name/container_name  or
...
class ResumingGetter(object):
    ...
    def _make_node_request(self, node, node_timeout, logger_thread_locals):
        ...
        try:
            ...
            # Timeout()可以向任何东西添加超时，在 timeout 秒后抛出异常
            with Timeout(node_timeout):
                # possible_source是一个 HTTPResponse 实例
                # HTTPResponse:　read(); getheader(name[, default]); version; status; reason
        ...
        if self.is_good_source(possible_source):
            ...
            if not float(possible_source.getheader('X-PUT-Timestamp', 1)): # float()函数用于将整数和字符串转换成浮点数
                ...
                # 取出 possible_source 中的 headers 部分保存在 src_headers 中
                # src_headers =
                # { 'content-length': '14', 'x-backend-timestamp': '****',
                #   'last-modified': '*****', 'connection': 'close', 'etag': '"*******"',
                #   'x-timestamp': '******', 'x-backend-data-timestamp': '*******',
                #   'date': '******', 'x-backend-durable-timestamp': '******',
    ...
    def _get_source_and_node(self):
        ...
        for node in nodes:
            ...
            if pile.waitfirst(_timeout):  # 如果在 _timeout 时间内有返回值
                break
        else:
...
class GetOrHeadHandler(ResumingGetter):
    ...
    def get_working_response(self, req):
        # source:
        # node: {'index': 2, 'replication_port': 6200, 'weight': 100.0, 'zone': 1,
        #        'ip': '10.0.0.9', 'region': 1, 'id': 2, 'replication_ip': '10.0.0.9',
...
class Controller(object):
    ...
    def container_info(self, account, container, req=None):
        ...
        """
        # part: an integer
        # nodes: a list of dict. eg:[{'index': 0, ......}, {'index': 1,.....},{......}, {.......}]
